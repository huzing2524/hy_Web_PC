    ...
    .content.fbox
      ...
      .right_box
        .info
          .name
            .title 看板名称
            el-input(placeholder="填写看板名称" v-model="listItem.name")
          .date
            .title 时间单位
            .date_box
          ...
          .style_list
            .item(:class="styleIndex === 1 ? 'checked' : ''" @click="styleOptions(1)")
              .icon(v-if="styleIndex === 1")
                svg.ali_icon(aria-hidden="true")
                  use(xlink:href="#iconzhe_white")
              .icon(v-else)
                svg.ali_icon(aria-hidden="true")
                  use(xlink:href="#iconzhe_blue")
              .info
                p 折线图
                p 显示数据的变化趋势
            .item(:class="styleIndex === 0 ? 'checked' : ''" @click="styleOptions(0)")
              .icon(v-if="styleIndex === 0")
                svg.ali_icon(aria-hidden="true")
                  use(xlink:href="#iconzhu_white1")
              .icon(v-else)
                svg.ali_icon(aria-hidden="true")
                  use(xlink:href="#iconzhu_blue1")
              .info
                p 柱状图
                p 比较数据之间的差别
        .data_select
...
<script>
  ...
  import { analysisChart, analysisChartDeviceItems } from '_api/data_manage'
    ...
    methods: {
      ...
      checkData(index, isChecked) {
        this.deviceItems.forEach((item, indexs) => {
          if (index === indexs) {
            this.$set(this.deviceItems, indexs, Object.assign(item, { isChecked: !isChecked }))
          }
        })
      },
      saveClick() {
        ...
        console.log(params)
        analysisChart(params, 'post', `?customer_id=${this.customer_id}&site_id=${this.site_id}&type=1`).then((res) => {
          if (res.data.code === 200) {
            this.loading = false
